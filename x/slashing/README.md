...
At the beginning of each block, we check each validator against the liveness
threshold over a sliding window. This sliding window is defined by
`SignedBlocksWindow` and the index in this window is determined by
`height - ValidatorSigningInfo.StartHeight`. The position in the sliding window
therefore advances every block without writing `ValidatorSigningInfo` solely to
move an index cursor. Once the index is determined, the `MissedBlocksBitArray`
and `MissedBlocksCounter` are updated accordingly.
...
for vote in block.LastCommitInfo.Votes {
  ...
  modifiedSignInfo := missedPrevious != missed
